        ...
        columns: [
            {
                ...
                classList: "scientific-name",
                href: function (item) {
                    return "http://www.artsportalen.artsdatabanken.no/#/Rodliste2010/Vurdering/" + item.scientificName.replace(" ", "_") + "/" + item.rodlistevurderingID;
                }
            },
            {headerText: "Populærnavn", isSortable: true, rowText: "vernacularName"},
            {headerText: "Kategori", isSortable: true, rowText: "kategori"},
            {
                headerText: "Hovedhabitat", rowText: function (item) {
                return item.hovedhabitat.join(", ");
            }
            },
    ...
    var filterSearch = function (listOfItems) {
        ...
        return _.filter(listOfItems, function (item) {
            return _.every(terms, function (term) {
                ...
                if (item.kategori.toLowerCase().indexOf(term) === 0) {
                    return true;
                }
                if (item.ekspertgruppe.toLowerCase().indexOf(term) === 0) {
                    return true;
                }
                if (item.scientificName.toLowerCase().indexOf(term) !== -1) {
                    return true;
                }
                if (item.vernacularName.toLowerCase().indexOf(term) === 0) {
                    return true;
                }
                return _.some(item.hovedhabitat, function (habitat) {
                    if (habitat.toLowerCase().indexOf(term) === 0) {
                        return true;
                    }
                });
            });
    ...
    return {
        ...
        activate: function () {
            ...
            return http.get("./viewmodels/ansvarsarter-data.json").then(function (response) {
                ...
                var unsortedKategorier = _(response).map(function (d) {
                    return d.kategori.substring(0, 2);
                }).uniq();//EN0 skal bli EN

                var sortedKategorier = unsortedKategorier.sortBy(function (item) {
                    return kategoriRekkefølge.indexOf(item);
                }).value();
